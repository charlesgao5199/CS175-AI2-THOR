"""Run a coverage-oriented ObjectNav baseline episode in AI2-THOR."""
...
def main() -> None:
    ...
    try:
        summary = run_coverage_episode(
            ...
            loop_window=int(config.get("loop_window", 8)),
            revisit_threshold=int(config.get("revisit_threshold", 3)),
            cell_size=float(config.get("cell_size", 0.25)),
            recorder=recorder,
        )
    finally:
        env.stop()
